...
export default function ChunkingReader({ words, currentIndex, chunkSize = 3 }) {
  ...
  return (
    <div className="min-h-[300px] w-full bg-background-elevated rounded-lg border border-border flex items-center justify-center">
      <div className="flex items-center gap-6 md:gap-8 lg:gap-10">
        {currentChunk.map((word, idx) => (
          <span
            key={`${word}-${idx}`}
            className="font-semibold transition-all duration-200 text-3xl md:text-5xl text-text-primary"
          >
            {word}
          </span>
        ))}
