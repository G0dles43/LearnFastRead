...
export default function TrainingSession({ api }) {
  ...
  const handleCheating = useCallback(async (reason) => {
    ...
    const calculatedTime = words.length * speed; 

    try {
      await api.post("submit-progress/", {
        exercise: id,
        reading_time_ms: calculatedTime, 
        answers: {}, 
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    async function fetchExerciseData() {
      try {
        ...
        if (exerciseData.is_ranked) {
          const statusRes = await api.get(`exercises/${id}/attempt-status/`);
    ...
    fetchExerciseData();
  }, [id, token, api, isLoadingSettings]);
  ...
  const finishExercise = () => {
    ...
    if (exercise?.is_ranked && attemptStatus?.can_rank) {
      ...
      api.post("submit-progress/", {
        exercise: id,
        reading_time_ms: calculatedReadingTimeMs,
        answers: {},
      })
        .then(res => {
