...
  // === FUNKCJA POBIERAJĄCA DANE UŻYTKOWNIKA (ZMIENIONA) ===
...
      setLoading(false);
    }
  }, [api]); // Usunięto 'navigate' z zależności, bo nie jest tu potrzebne
...
  // === FUNKCJA ZAPISU PROFILU (ZMIENIONA) ===
...
        fileInputRef.current.value = ""; // Wyczyść pole input
      }
      // 2. Mówimy frontendowi, aby OD NOWA pobrał dane z serwera.
      // To da nam 100% pewności, że mamy nowy URL awatara (z cache-busterem).
      await fetchUserData(); 
      // === KONIEC POPRAWKI ===
...
  // Obsługa zmiany pól w formularzu hasła
  const handlePasswordChange = (e) => {
    const { name, value } = e.target;
    setPasswordData(prev => ({ ...prev, [name]: value }));
  };

  // Wysyłanie formularza ZMIANY HASŁA
